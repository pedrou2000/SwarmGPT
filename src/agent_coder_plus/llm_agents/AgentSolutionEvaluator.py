...
class AgentSolutionEvaluator(CodeInterpreterAgent):
    system_prompt: str = prompts.META_AGENT_CODER["AgentSolutionEvaluator"]["system"]
    test_executor_prompt: str = prompts.META_AGENT_CODER["AgentSolutionEvaluator"]["user"]
    parser_system_prompt: str = prompts.META_AGENT_CODER["AgentSolutionEvaluator"]["parser_system"]
    ...
    def __call__(self, state) -> Any:
        ...
        for completed_method in state.final_answers:
            prompt_args = {
            ...
            input_to_parse = completed_method + state.generated_tests + agent_response
            ...
            proportion_tests_passed_dict[completed_method] = proportion_tests_passed
        ...
        for key, value in proportion_tests_passed_dict.items():
            print(f"Proportion tests passed: {value} for method: \n{key}\n")
        print(f'\n'  + '-'*20)
